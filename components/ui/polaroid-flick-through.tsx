...
const PLACEHOLDER_BACKGROUNDS = [
  'from-[#dcc2a7] via-[#f2e8db] to-[#e6d1bd]',
  'from-[#b6c7d3] via-[#e7eef2] to-[#d4dee6]',
  'from-[#d7b9bf] via-[#f4e6e8] to-[#e8cdd1]',
];
...
export function PolaroidFlickThrough({
  ...
  return (
    <section
      ref={containerRef}
      className={`relative h-[420vh] bg-[#f7f0e6] ${className}`}
    >
                ...
                <div className="relative h-[430px] w-full max-w-[280px] sm:h-[520px] sm:max-w-[420px] lg:h-[560px] lg:max-w-[460px]">
                  {cards.map((card, index) => {
                    ...
                    return (
                      <motion.figure
                        key={card.id}
                        className="absolute left-1/2 top-1/2 w-[240px] -translate-x-1/2 -translate-y-1/2 rounded-[6px] border border-black/8 bg-[#fffdf8] p-2.5 shadow-[0_18px_45px_rgba(57,39,25,0.16)] sm:w-[280px] sm:p-3 lg:w-[330px]"
                        animate={state}
                        transition={{
                          type: 'spring',
                          stiffness: 170,
                      ...
                      >
                        <button
                          ...
                          className="group relative block w-full cursor-pointer rounded-[4px] text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3c2d24]/60"
                        ...
                        >
                          ...
                          >
                            <div className="rounded-[3px] bg-[#f4ebdf] p-2 transition-shadow duration-300 group-hover:shadow-[0_14px_36px_rgba(57,39,25,0.18)]">
                              <div className="relative aspect-[4/5] overflow-hidden rounded-[2px] bg-[#ece0d0]">
                                {card.imageSrc ? (
                                  <Image
                                    src={card.imageSrc}
                                  ...
                                  <div
                                    className={`flex h-full w-full flex-col justify-between bg-gradient-to-br ${background} p-5 text-[#3f3128]`}
                                  >
                                    <span className="text-[0.65rem] font-medium uppercase tracking-[0.3em] text-black/45">
                                      Stage {card.stage}
                                    </span>
                                    <div>
                                      ...
                                      <p className="mt-3 max-w-[16ch] text-sm leading-relaxed text-black/55">
                                        Photo placeholder
                                      </p>
                                    </div>
                                  </div>
                            ...
                            <figcaption className="px-2 pb-1 pt-4 text-center">
                              <p className="font-serif text-xl tracking-[-0.03em] text-[#46362b]">
                                {card.note}
                              </p>
                              <p className="mt-1 text-[10px] font-medium uppercase tracking-[0.28em] text-black/35 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                                Click to view
                              </p>
                            </figcaption>
                          </motion.div>
          ...
          >
            ...
            >
              <button
                type="button"
                onClick={closeViewer}
                className="absolute right-3 top-3 z-10 grid h-10 w-10 place-items-center rounded-full bg-white/95 text-black shadow-md transition hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white sm:right-4 sm:top-4"
                aria-label="Close"
              >
                ✕
              </button>

              <div className="overflow-hidden rounded-xl bg-[#fffdf8] p-4 pt-7 shadow-[0_30px_80px_rgba(0,0,0,0.45)] sm:pt-4">
                <div className="relative mx-auto aspect-[4/5] w-full max-w-[520px] overflow-hidden rounded-lg bg-[#ece0d0]">
                  {activeCard.imageSrc ? (
                    <Image
                    ...
                    <div className="flex h-full w-full flex-col items-center justify-center px-8 text-center text-[#3f3128]">
                      <p className="font-serif text-3xl tracking-[-0.04em]">{activeCard.title}</p>
                      <p className="mt-3 text-sm text-black/60">No image provided.</p>
                    </div>
                  )}
                </div>

                <div className="pt-4 text-center">
                  <p className="font-serif text-2xl tracking-[-0.03em] text-[#46362b]">
                    {activeCard.note}
                  </p>
                </div>
              </div>
...
function StageCopy({
  ...
  return (
    ...
    >
      ...
      <h3 className="font-serif text-[2rem] leading-none tracking-[-0.04em] text-[#3c2d24] sm:text-3xl">
        {stage.title}
      </h3>
      <p className="text-sm leading-6 text-black/55 sm:leading-7">{stage.description}</p>
    </motion.div>
  );
}
